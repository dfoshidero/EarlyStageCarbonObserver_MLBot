...
def tune_model(X, y):
    ...
    for model_name, (model, param_dist) in models.items():
        ...
        param_space_size = np.prod([len(v) if hasattr(v, '__len__') else 1 for v in param_dist.values()])
        n_iter = min(50, param_space_size)

        random_search = RandomizedSearchCV(estimator=pipeline, param_distributions=param_dist, n_iter=n_iter, cv=5, n_jobs=-1, scoring='r2', random_state=42)
...
def load_datasets():
    ...
    becd_df_PATH = os.path.join(import_dir, 'cleaned_becd.csv')
    carbenmats_df_PATH = os.path.join(import_dir, 'cleaned_carbenmats.csv')
    clf_df_PATH = os.path.join(import_dir, 'cleaned_clf.csv')
    
    becd_df = pd.read_csv(becd_df_PATH)
    carbenmats_df = pd.read_csv(carbenmats_df_PATH)
    clf_df = pd.read_csv(clf_df_PATH)
    
    return becd_df, carbenmats_df, clf_df
...
def prepare_datasets(becd_df, carbenmats_df, clf_df, target_column='Total Embodied Carbon PER m2'):
    becd_df, becd_label_encoders = encode_categorical(becd_df)
    carbenmats_df, carbenmats_label_encoders = encode_categorical(carbenmats_df)
    clf_df, clf_label_encoders = encode_categorical(clf_df)
    ...
    return (X_becd, y_becd, becd_label_encoders), (X_carbenmats, y_carbenmats, carbenmats_label_encoders), (X_clf, y_clf, clf_label_encoders)
